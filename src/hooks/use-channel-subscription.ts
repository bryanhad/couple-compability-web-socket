...
import { CreatorHasBeenWaiting_EventPayload, triggerCreatorHasBeenWaitingEvent } from "@/server-actions/trigger-creator-has-been-waiting-event"
...
export function useChannelSubscription(currentRoomId: string) {
    ...
    const { pusherClient, userInfo, partnerInfo, selectedQuestionKeys, setPartnerInfo, setUserInfo, setChannelMemberCount, setSelectedQuestionKeys } =
        useClientContext()
    ...
    useEffect(() => {
        ...
        const presenceChannel = pusherClient.subscribe(`presence-${currentRoomId}`) as PresenceChannel

        channel.bind(JOINER_JOINED_ROOM_EVENT, ({ joinerName }: JoinerJoinedRoom_EventPayload) => {
        ...
        channel.bind(CREATOR_HAS_BEEN_WAITING_EVENT, ({ creatorName, selectedLanguage, selectedQuestionKeys }: CreatorHasBeenWaiting_EventPayload) => {
        ...
        channel.bind(CREATOR_SUBMITTED_FORM_EVENT, ({ formValues: creatorAnswers }: CreatorSubmittedForm_EventPayload) => {
            if (!userInfo) {
                return
            }
            if (userInfo.role === "joiner") {
                ...
                setUserInfo((prev) => (prev ? { ...prev, formValues: creatorAnswers } : null))
            }
        })
        channel.bind(JOINER_SUBMITTED_FORM_EVENT, ({ formValues: joinerAnswers }: JoinerSubmittedForm_EventPayload) => {
            if (!userInfo) {
                return
            }
            if (userInfo.role === "creator") {
                ...
                setUserInfo((prev) => (prev ? { ...prev, formValues: joinerAnswers } : null))
            }
        })
